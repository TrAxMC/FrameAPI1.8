...
public class MySQLMoney implements Listener {
        ...
        if(FrameMain.getInstance().getConfig().getBoolean("MYSQL.Boolean")) {
            if(cfgMoney.getBoolean(player.getUniqueId() + "." + player.getName() +".registered")) {
                ...
                if(IsTableExist.isExist("money")) { //Look if the table is exists
                    ...
                    String sql2 = "CREATE TABLE IF NOT EXISTS money(PlayerName TEXT(11),balance_money INT);"; // Creates The Table
                    try {
                        PreparedStatement stmt = (PreparedStatement) MYSQL.getConnection().prepareStatement(sql2);
                        stmt.executeUpdate();
                        // Insert the Players when it is not Exists in MYSQL
                        stmt.executeUpdate("INSERT INTO money (PlayerName,balance_money) VALUES ('"+player.getName()+ "','" + 0 + "');");
                        stmt.executeUpdate();
    ...
    public void setRegistered(boolean registered) {
        MySQLMoney.registered = registered;
    }
    /**
     * @param player Player
    ...
    private void saveMoneyInSQL(OfflinePlayer player, double amount) {
        ...
        try {
            ...
            if(IsTableExist.isExist("money")) {
                ...
                ResultSet res = statement.executeQuery("SELECT * FROM money WHERE PlayerName = '" + player.getName() + "';");
                if(res.next()) {
                    if(res.getString("PlayerName") == null) {
                        statement.executeUpdate("INSERT INTO money (PlayerName,balance_money) VALUES ('"+player.getName()+ "','"+amount+"');");
                    } else {
                        String sql = "UPDATE money SET balance_money = '" + amount +"' WHERE PlayerName = '" + player.getName() + "'";
    ...
    private Double getMoneyMySql(OfflinePlayer player) {
        ...
        if(IsTableExist.isExist("money")) {
            try {
                ...
                ResultSet res = statement.executeQuery("SELECT * FROM money WHERE PlayerName = '" + player.getName() + "';");
                if(res.next()) {
                    int x = res.getInt("balance_money");
    ...
    public void SaveMoneyInBank(OfflinePlayer player, double amount) {
        ...
        try {
            ...
            if(IsTableExist.isExist("BankMoney")) {
                ...
                try {
                    PreparedStatement stmt = (PreparedStatement) MYSQL.getConnection().createStatement();
